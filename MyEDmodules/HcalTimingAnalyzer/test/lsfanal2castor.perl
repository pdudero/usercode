...
$castorDir="/castor/cern.ch/user/d/dudero/pgun2smearv1";
...
$jobBase=~s/[.]py$//; # remove trailing ".py" suffix
...
for ($i=0; $i<=$#ARGV; $i++) {
    ...
    $stub=$infname."-smear";
    $jobCfg="$prodSpace/$jobBase/cfg/".$stub.".py";

    $outfname=specializeCfg($template,$jobCfg,$stub);
    $stub=$jobCfg;
    $stub=~s|.*/([^/]+).py$|$1|;
    ...
    print(SUBMIT "rfcp $outfname $castorDir;\n");
    close(SUBMIT);
}
