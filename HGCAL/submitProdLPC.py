...
#    if et>0 and et<60 : myqueue=opt.squeue
    ...
    scriptFile.write('./PFCalEE g4steer.mac %d %d %f > g4.log\n'%(opt.version,opt.model,opt.eta))
    ...
    # Condor jdl file
    jdlFile = open('%s/submit.jdl'%(outDir), 'w')
    ...
    jdlFile.write('transfer_input_files = g4env4lpc.sh,%s/g4steer.mac,%s/bin/Linux-g++/PFCalEE,%s/tmp/Linux-g++/PFCalEE/libPFCalEE.so,analysis/lib/libPFCalEEAnalysis.so,userlib/lib/libPFCalEEuserlib.so\n'%(outDir,os.environ['G4WORKDIR'],os.environ['G4WORKDIR']))
    jdlFile.write('WhenToTransferOutput = ON_EXIT\n')
    jdlFile.write('Error = pfcalee.stderr\n')
    jdlFile.write('Output = pfcalee.stdout\n')
    jdlFile.write('Queue\n')
    jdlFile.close()
    
    #submit
    os.system('chmod u+rwx %s/runJob.sh'%outDir)
    if opt.nosubmit : os.system('echo condor_submit %s/submit.jdl'%outDir) 
    else: os.system('condor_submit %s/submit.jdl'%outDir)
